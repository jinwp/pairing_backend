...
export class LoveAlarmService {
  ...
  // pseudo‑code – drop into LoveAlarmService
  async getCrushCount(
    targetId: number,
    latitude: number,
    longitude: number,
    ...
    return this.userRepository
      .createQueryBuilder('lover')
      .innerJoin('lover.love', 'target', 'target.id = :targetId', { targetId })
      .innerJoin(
        qb => qb
          .select('DISTINCT ON(l.userId) l.*')   // grab the latest point each user saved
          .from(Location, 'l')
          .orderBy('l."userId", l."timestamp"', 'DESC'),
        'loc',
        'loc."userId" = lover.id',
      )
      .where(
        `ST_DWithin(
            loc.point::geography,
            ST_GeomFromText(:targetPoint, 4326)::geography,
            :radius
        )`,
        { targetPoint, radius },
      )
